// ═══════════════════════════════════════════════════════════════════
// HELPERS — 100% copy from web app (src/utils/helpers.js)
// Only change: TypeScript annotations added
// ═══════════════════════════════════════════════════════════════════

import type { User, UserRole, CustomTheme } from '../types';
...
export const SEASONS: Record<string, CustomTheme> = {
...
// ═══════════════════════════════════════════════════════════════════
export const ROLES = [
  { value: 'member' as const, label: 'Członek/Członkini' },
  { value: 'zarzad' as const, label: 'Zarząd' },
  { value: 'skarbnik' as const, label: 'Skarbnik' },
  { value: 'admin' as const, label: 'Administrator' },
];

export function canSeeDues(r: UserRole): boolean {
  return ['admin', 'skarbnik', 'zarzad'].includes(r);
}

export function canManage(r: UserRole): boolean {
  return ['admin', 'zarzad', 'skarbnik'].includes(r);
}

export function isAdmin(r: UserRole): boolean {
  return r === 'admin';
}
...
export function fDate(d: string | undefined | null): string {
  if (!d) return '';
  const x = new Date(d);
  return `${x.getDate()} ${MO_S[x.getMonth()]} ${x.getFullYear()}`;
}

export function mName(m: string): string {
  const [y, mo] = m.split('-');
  return `${MO_PL[parseInt(mo) - 1]} ${y}`;
}

export function ini(n: string | undefined | null): string {
  return (n || '?').split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
}

export function avC(id: string): string {
  let h = 0;
  for (let i = 0; i < String(id).length; i++) {
    h = ((h << 5) - h) + String(id).charCodeAt(i);
    h |= 0;
  }
  return AV_C[Math.abs(h) % AV_C.length];
}
...
export function timeAgo(d: string): string {
  ...
  if (s < 84400) return `${Math.floor(s / 3600)} godz. temu`;
  if (s < 604800) return `${Math.floor(s / 86400)} dn. temu`;
  return fDate(d);
}
...
export function tplt(str: string | undefined | null, data: Record<string, string | number>): string {
  if (!str) return '';
  return str.replace(/\{\{(.*?)\}\}/g, (match, key) => String(data[key.trim()] ?? match));
}

export function uName(usersList: User[], aid: string, orgName = 'System'): string {
  if (aid === 'seed' || aid === 'system') return orgName;
  if (!usersList) return 'Anonim';
  if (aid.startsWith('guest_')) return aid.replace('guest_', '') + ' (Gość)';
  const u = usersList.find(x => x.id === aid);
  return u ? u.name : 'Nieznany';
}

export function mailWrapper(text: string | undefined | null): string {
  if (!text) return '';
  const formattedText = text.replace(/\n/g, '<br>');
  return `<div style="background-color: #f4f8fb; padding: 30px; font-family: Arial, sans-serif;"><div style="background-color: #ffffff; padding: 24px; border-radius: 12px; border: 1px solid #e2e8f0; max-width: 600px; margin: 0 auto; color: #333; line-height: 1.6; font-size: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">${formattedText}</div></div>`;
}
...
export function balanceTeams(
  attendees: string[],
  allUsers: User[],
  customRatings: Record<string, number | string> = {}
) {
  const players = attendees.map(id => {
    ...
    return { id, name: u?.name || id.replace('guest_', ''), rating: finalRating };
  });
...
export const APP_PRESETS = [
  {
    ...
    recipeFields: { cats: ['Taktyka', 'Trening', 'Stałe fragmenty', 'Analiza', 'Inne'], ingrLabel: 'Elementy / Sprzęt', instrLabel: 'Opis taktyki / Przebieg' },
  },
  {
    ...
    modules: { modDues: true, modIdeas: true, modRecipes: true, modEvents: true, modGallery: true, modDocuments: true, modFinances: true },
    recipeFields: { cats: ['Ratownictwo', 'Gaśnicze', 'Sprzęt', 'Medyczne', 'Inne'], ingrLabel: 'Wymagany sprzęt', instrLabel: 'Procedura operacyjna' },
  },
  {
    ...
    modules: { modDues: true, modIdeas: true, modRecipes: true, modEvents: true, modGallery: true, modDocuments: true, modFinances: true },
    recipeFields: { cats: ['Ciasta', 'Zupy', 'Dania główne', 'Sałatki', 'Przetwory', 'Inne'], ingrLabel: 'Składniki', instrLabel: 'Sposób przygotowania' },
  },
  {
    ...
    recipeFields: { cats: ['Artykuły', 'Poradniki', 'Interpretacje', 'Materiały', 'Inne'], ingrLabel: 'Załączniki / Odnośniki', instrLabel: 'Treść' },
  },
  {
    ...
    recipeFields: { cats: ['Procesy', 'Onboarding', 'Materiały', 'Narzędzia', 'Inne'], ingrLabel: 'Zasoby', instrLabel: 'Opis procedury' },
  },
  {
    ...
    modules: { modDues: true, modIdeas: true, modRecipes: true, modEvents: true, modGallery: true, modDocuments: true, modFinances: true },
    recipeFields: { cats: ['Poradniki', 'Materiały', 'Inne'], ingrLabel: 'Wymagania', instrLabel: 'Instrukcja' },
  },
];
